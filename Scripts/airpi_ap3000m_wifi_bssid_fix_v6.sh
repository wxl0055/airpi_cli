...
echo "Apply Airpi AP3000M WiFi BSSID fix v6"
echo "WRT=$WRT"
...
logger -t ap3000m-mtwifi-mac-sanitize "2G=$M2 5G=$M5"
sync
...
grep -nE '^BssidNum=|^MacAddress=|^MacAddress1=|^MacAddress2=' \
...
hook = '    os.execute("/usr/sbin/ap3000m-mtwifi-mac-sanitize >/dev/null 2>&1")'

for name in sys.argv[1:]:
    p = Path(name)
    s = p.read_text()

    if "ap3000m-mtwifi-mac-sanitize" not in s:
        s = s.replace(
...
grep -RniE 'ap3000m-mtwifi-mac-sanitize|save_profile\(dats, profile\)' "${CFGS[@]}" || true
...
echo "Airpi AP3000M WiFi BSSID fix v6 applied."
